...
#try(bern_kt <- read.dta("H:/Assessing Inequalty/taxDataBE2012.dta"))
try(bern_kt<-read.csv("U:/Persönlich/Forschung/Uni Bern/BFH-Grunddaten 2012 Output V1.1.csv",sep=";"))
...
bern <- read.dta("G:/stick/bern/mini_taxdata_BE_all.dta")
bern_gini <- bern %>%
...
# legend order
levels(bern_gini_long$Income) <- c("total income", "taxable income", "taxable income after federal income tax", "taxable income after all income taxes")
bern_gini_long$Income <- factor(bern_gini_long$Income,levels=c("taxable income", "taxable income after federal income tax", "taxable income after all income taxes","total income"))
...
fig_bern_gini <- ggplot(bern_gini_long, aes(x=stj,y=Gini)) +
  ...
  xlab("Year")+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))+
  ggtitle("Gini coefficients of different income measures")
fig_bern_gini
...
estv <- read.dta("../../data/steuerdaten20141126_stata12.dta")
