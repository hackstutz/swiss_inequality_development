...
density1 <- density(bd.2003.ind$inc)
...
legend(fig1legend,lty=1:2,cex=1, bty="n",
       legend=c("2003","2010"))
density2 <- density(bd.2010.ind$inc)
lines(x = (density2$x), y = density2$y, type = "l",lty=2)

# Variante mit Log

density1 <- density(log(bd.2003.ind$inc))
plot(x = (density1$x), y = density1$y, type = "l",
...
legend(fig1legend,lty=1:2,cex=0.5, bty="n",
       legend=c("2003","2010"))
density2 <- density(log(bd.2010.ind$inc))
lines(x = (density2$x), y = density2$y, type = "l",lty=2)
...
abline(h=1,lty=2)
gA0 <- reldist(y=bd.2010.ind$inc, yo=bd.2003.ind$inc,
               smooth=0.4, ci=FALSE,
               show="residual",
               bar=TRUE, quiet=FALSE,
               ylim=c(0.5,5.0), ylab="",
...
ggplot(dflong[dflong$kanton=="CH" & dflong$steuerperiode>=1995.5,], aes(x=steuerperiode,y=Gini,shape=Type,linetype=Type))+geom_line(size=1)+theme_bw()+xlab("tax period")+theme(legend.justification=c(0,1),legend.position=c(0, 1))
...
axis(side = 1)
axis(side = 2)
fig1legend <- list(x=c(800,800),y=c(1.097e-02,1.097e-02))
legend(fig1legend,lty=1:2,cex=1, bty="n",
       legend=c("HBS","FTA"))
...
title(main=paste("Overall relative density"))
abline(h=1,lty=2)
...
plot(x = (density1$x), y = density1$y, type = "l",
     xlab = "Einkommen", ylab = "density",
     axes = FALSE,
     xlim = c(min(rbind(density1$x,density2$x)), max(rbind(density1$x,density2$x))),
