...
try(bern_kt <- read.csv("G:/stick/bern//BFH-Grunddaten 2012 Output V1.1.csv",sep=";"))
#> nrow(bern_kt)
#[1] 622368
...
bern <- filter(bern_kt,Satzeink_BUND<=TOTEINK,erm==0)
bern <- filter(bern_kt,erm==0)
...
bern_gini <- bern %>%
  group_by(stj) %>%
    summarise(G_TOTEINK = gini(TOTEINK),
            G_satzeink_BUND = gini(Satzeink_KANTON),
