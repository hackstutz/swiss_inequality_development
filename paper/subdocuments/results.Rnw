...
bd<-subset(bd,(Jahr<=1995 | Jahr >2002)&Veranlagungsperiode!=1995)
# 2011 zufügen
...
bd.0<-bd.0 %>% 
  filter(Einheit=="Total") %>% select(Veranlagungsperiode,Jahr,gini_steink,gini_steinka)
bd.0<-subset(bd.0,(Jahr<=1995 | Jahr >2002)&Veranlagungsperiode!=1995)
# 2011 zufügen
...
dflong$Gini[dflong$kanton=="CH"&dflong$bemessungsjahre<1995&dflong$Type=="including non-taxed"]<-NA
dflong$Gini[dflong$kanton=="CH"&dflong$bemessungsjahre<1995&dflong$Type=="imputed values for non-taxed"]<-NA
fig_zeros <- ggplot(dflong[dflong$kanton=="CH",], 
