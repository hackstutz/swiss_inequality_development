...
fig_zeros <- ggplot(dflong[dflong$kanton=="CH",], 
  ...
  scale_y_continuous(limits=c(0.28,0.55))+
  scale_x_continuous(limits=c(2003,2012),breaks=number_ticks(10)) +
  geom_line(size=0.5)+
...
bern_kt2012 <- bern_kt %>% filter(stj==2012) %>% group_by(hh_id) %>% select(tot_erwerbeink,tot_vermeink,primeink,tot_transeink,TOTEINK,hh_id)
...
bern_kt2012 <- bern_kt2012_plain %>% mutate(primeink_hh=sum(primeink)) %>% filter(!duplicated(hh_id)) #nimmt die erste unique beobachtung und droppt 2. bis x. 
...
# export nach stata (ungewichtet)
bern_plain_hh <- rbind(data.frame("income" = bern_kt2012_plain$primeink/1000,"group"=0,"weight"=1),data.frame("income" = bern_kt2012$primeink_hh/1000,"group"=1,"weight"=1))
...
fig_measures <- ggplot(estv_masse_long, aes(x=Year,y=log(value),shape=measure))+
  #scale_y_continuous(limits=c(0,0.55))+
  scale_x_continuous(limits=c(1918,2011),breaks=number_ticks(10)) +
...
grid.arrange(fig_measures,fig_equiv,fig_incomes,fig_zeros)
dev.off()
@
