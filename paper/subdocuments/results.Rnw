...
bern_kt2012 <- bern_kt2012_plain %>% mutate(TOTEINK_hh=sum(TOTEINK)) %>% filter(!duplicated(hh_id)) #nimmt die erste unique beobachtung und droppt 2. bis x. 
...
bern_habe <- rbind(habe11_bern,data.frame("income" = bern_kt2012$TOTEINK_hh/1000,"group"=1,"weight"=1))
...
# export nach stata (ungewichtet)
bern_plain_hh <- rbind(data.frame("income" = bern_kt2012_plain$TOTEINK/1000,"group"=0,"weight"=1),data.frame("income" = bern_kt2012$TOTEINK_hh/1000,"group"=1,"weight"=1))
...
estv <- read.dta("../../data/steuerdaten20141118_stata12.dta")
