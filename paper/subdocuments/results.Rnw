...
fig_equiv
# Gini Range
max(bd %>% mutate(d_steink=abs(gini_steink-gini_steinka),d_reink=abs(gini_reink-gini_reinka)) %>% select(starts_with("d_")),na.rm=TRUE)
# 0.011
@
...
bd2010_2003 <- rbind(data.frame(cbind("bd" = bd.2010.ind,"group"=2010)),data.frame("bd" = bd.2003.ind,"group"=2003))
write.dta(bd2010_2003, file="../../data/stata_data/bd2010_2003.dta")
@
...
# Export data to use with Stata
ns2010 <- rbind(data.frame(cbind("income" = pooled_i,"group"=0)),data.frame("income" = normal_i,"group"=1))
ns2010$group <- factor(ns2010$group,labels=c("pooled","normal"))
write.dta(ns2010, file="../../data/stata_data/ns2010.dta")

#Gini Range
gini_allcases <- gini(ns2010$income[ns1993_94$group=="pooled"]); gini_allcases
gini_normal <- gini(ns2010$income[ns1993_94$group=="normal"]); gini_normal
round(gini_allcases-gini_normal,3)
# 0.025
...
habe11_bern <- habe0911 %>% filter(Kanton08==2) %>% 
  mutate(income=(Bruttoeinkommen08+A31m)*12/1000,weight=jahresgewicht,group=0) %>% 
...
fig_measures <- ggplot(estv_masse_long, aes(x=Year,y=log_value,shape=measure))+
  ...
  scale_x_continuous(limits=c(1917,2011),breaks=number_ticks(10)) +
