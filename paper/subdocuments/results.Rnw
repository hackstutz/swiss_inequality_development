...
bd<-bd %>% 
  filter(Einheit=="Total") %>% select(Veranlagungsperiode,Jahr,gini_reink,gini_reinka)
bd<-subset(bd,Jahr<1995 | Jahr >2002)
# 2011 zufügen
bd.2011 <- read.csv("../../data/NP_2011.csv",dec=",") %>% filter(Einheit=="Total") %>% mutate(Veranlagungsperiode=2011,Jahr=2011)
...
# 2011 zufügen
bd.0.2011 <- read.csv("../../data/NP_2011_mitnull.csv",dec=",") %>% filter(Einheit=="Total") %>% mutate(Veranlagungsperiode=2011,Jahr=2011)
levels(bd.0$Veranlagungsperiode)<- c(levels(bd.0$Veranlagungsperiode),"2011")
...
fig_equiv
# Gini Range
max(bd %>% mutate(d_reink=abs(gini_reink-gini_reinka)) %>% select(starts_with("d_")),na.rm=TRUE)
max(bd.0 %>% mutate(d_reink=abs(gini_reink-gini_reinka)) %>% select(starts_with("d_")),na.rm=TRUE)
# 0.012
@
