...
set0 <- function(x) ifelse(x<0,0,x)
...
bern_kt2012 <- bern_kt %>% filter(stj==2012) %>% group_by(hh_id) %>% select(tot_erwerbeink,tot_vermeink,primeink,primeink2,tot_transeink,TOTEINK,Satzeink_BUND,hh_id)
...
bern_kt2012_plain <- bern_kt2012  %>% filter(n()<=7) #5% werden gefiltert
#bern_kt2012 <- bern_kt2012_plain %>% mutate(TOTEINK_hh=sum(TOTEINK)) %>% filter(!duplicated(hh_id))
bern_kt2012 <- bern_kt2012_plain %>% mutate(primeink_hh=sum(primeink),primeink2_hh=sum(primeink2),Satzeink_BUND_hh=sum(Satzeink_BUND)) %>% filter(!duplicated(hh_id)) #nimmt die erste unique beobachtung und droppt 2. bis x. 
...
# export nach stata (ungewichtet)
bern_plain_hh <- rbind(data.frame("income" = bern_kt2012_plain$Satzeink_BUND/1000,"group"=0,"weight"=1),data.frame("income" = bern_kt2012$Satzeink_BUND_hh/1000,"group"=1,"weight"=1))
