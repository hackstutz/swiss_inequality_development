...
In this section we will present results following the strategy introducded in the last section. As describded this includes changeing possible concepts within one field (income definition, statistical unit, measurement and population coverage) while holding other conceptional differences constant. To get the best possible comparison we rely on different data sources (FTA-tax tax data and HBS-survey data) and different time periodes. The presentation of results therefore includes a description of the used data.
...
library(reshape)
df <- df %>% filter(kanton=="CH"&(bemessungsjahre<=1993.5|bemessungsjahre>=2003))
number_ticks <- function(n) {function(limits) pretty(limits, n)}
dflong <- melt(df[,c("G_steink","G_reink","G_taxed","kanton","bemessungsjahre")], id=c("kanton","bemessungsjahre"))
names(dflong)[3]<-"Income"
names(dflong)[4]<-"Gini"
levels(dflong$Income)<-c("taxable income (total income - all deductions)","net income (total income - non-social deductions)","taxable income after federal tax")
dflong$Income <- factor(dflong$Income,levels=c("taxable income (total income - all deductions)","taxable income after federal tax","net income (total income - non-social deductions)"))
fig_incomes <- ggplot(dflong[dflong$kanton=="CH",], aes(x=bemessungsjahre,y=Gini,shape=Income))+
  scale_y_continuous(limits=c(0.28,0.55))+
  scale_x_continuous(limits=c(1945,2011),breaks=number_ticks(10)) +
  geom_point(aes(shape=Income),size=pointsize)+
  geom_line(data=dflong[dflong$kanton=="CH",],aes(linetype=Income),size=linesize)+
  theme_bw()+
  xlab("Year")+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))+
  ggtitle("Income definitions within tax data")
fig_incomes
# was ist die maximale Bandbreite zwischen den Punkten?
max(filter(df,kanton=="CH") %>% mutate(d_steink_reink=abs(G_steink-G_reink),d_steink_taxed=abs(G_steink-G_taxed),d_reink_taxed=abs(G_reink-G_taxed)) %>% select(starts_with("d_")),na.rm=TRUE)
# 0.026

#ggplot(dflong, aes(x=steuerperiode,y=Gini,colour=Type))+geom_line()+facet_wrap(~kanton)+theme_bw()+xlab("tax period")
@

As \ref{fig:different_ginis} shows, the development for the three defined measures of income is quite parallel except for the 1980s. In this time periode the Gini-coefficent for net income veers. This has probably to do with a change in regulations of deductions and shows that interpretation over time has to be very careful, becauses changes in taxation or regulation systems can affect the outcome. In general inequality assessed with taxable income is higher than inequality assessed with net income or taxable income after federal taxes. While it is not surprising that federal taxes reduces inequality slightly because of the progressivity of the taxes it is somehow astonishing that adding deductions (from net income to taxable income) increases inequality. This implies deductions to be rather flat not progressive. 


\subsection{Statistical units -- Equivalence scale}

The main issue concerning the unit of analysis is not easy to solve with tax-data, because the concepts of tax-units and households are not perfectly congruent (we will take up this issue, when comparing survey data and the FTA tax data later on.) However we can examine, how the assessment of inequality is affected by the implementation of equivalence scale. We do this by looking at Gini-time series for net income with and without implementation of a equivalence scale. The scale is constructed by using information out of tax data. The incomes of single households are divided by 1 (no change), for married tax units the equivalence-factor is 1.5. For every child and person supported by the tax-unit a value of 0.3 is added to the denominator. These measures are provided directly by the FTA and are not calculated by us. Because excluding the group of not-taxed leads to a longer time-series we provide four time-series in total (two possibilities to compare the effect of a equivalence scale).
...
bd<-bd %>% 
  filter(Einheit=="Total") %>% select(Veranlagungsperiode,Jahr,gini_steink,gini_steinka)
bd<-subset(bd,(Jahr<=1993 | Jahr >2002)&Veranlagungsperiode!=1995)
# 2011 zufügen
bd.2011 <- read.csv("../../data/NP_2011.csv",dec=",") %>% filter(Einheit=="Total") %>% mutate(Veranlagungsperiode=2011,Jahr=2011)%>% select(Veranlagungsperiode,Jahr,gini_steink,gini_steinka)
levels(bd$Veranlagungsperiode)<- c(levels(bd$Veranlagungsperiode),"2011")
bd <- rbind(bd,bd.2011[,names(bd)])
bd.long<-reshape(bd,
varying = c("gini_steink","gini_steinka"),
v.names = "Gini",
timevar = "inc_def",
times = c("taxable income (only taxed)","equivalised taxable income (only taxed)"),
direction ="long")
# mit Nuller
...
bd.0<-bd.0 %>% 
  filter(Einheit=="Total") %>% select(Veranlagungsperiode,Jahr,gini_steink,gini_steinka)
bd.0<-subset(bd.0,(Jahr<=1993|Jahr>=2003)&Veranlagungsperiode!=1995)
# 2011 zufügen
bd.0.2011 <- read.csv("../../data/NP_2011_mitnull.csv",dec=",") %>% filter(Einheit=="Total") %>% mutate(Veranlagungsperiode=2011,Jahr=2011) %>% select(Veranlagungsperiode,Jahr,gini_steink,gini_steinka)
levels(bd.0$Veranlagungsperiode)<- c(levels(bd.0$Veranlagungsperiode),"2011")
bd.0 <- rbind(bd.0,bd.0.2011[,names(bd.0)])
bd.0.long<-reshape(bd.0,
varying = c("gini_steink","gini_steinka"),
v.names = "Gini",
timevar = "inc_def",
times = c("taxable income","equivalised taxable income"),
direction ="long")
bdlong<-rbind(bd.0.long,bd.long)
names(bdlong)[3]<-"Income"
bdlong$Income <- factor(bdlong$Income,levels=c("equivalised taxable income","taxable income","equivalised taxable income (only taxed)","taxable income (only taxed)"))
@

<<equivalencescale, fig.cap='Gini coefficients with and without equivalization',fig.pos='H'>>=
number_ticks <- function(n) {function(limits) pretty(limits, n)}
fig_equiv <- ggplot(bdlong, aes(x=Jahr,y=Gini,shape=Income))+
  scale_y_continuous(limits=c(0.28,0.55))+
  #scale_x_continuous(limits=c(1970,2012),breaks=number_ticks(10)) +
  scale_x_continuous(limits=c(1970,2011),breaks=number_ticks(10)) +
  geom_point(aes(shape=Income),size=pointsize)+
  geom_line(data=bdlong,aes(linetype=Income),size=linesize)+
  theme_bw()+
  xlab("Year")+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))+
  ggtitle("Using (pseudo) equivalence scales")
fig_equiv
# Gini Range
max(bd %>% mutate(d_reink=abs(gini_steink-gini_steinka)) %>% select(starts_with("d_")),na.rm=TRUE)
max(bd.0 %>% mutate(d_reink=abs(gini_steink-gini_steinka)) %>% select(starts_with("d_")),na.rm=TRUE)
# 0.011
@
...
bd.0$Jahr <- as.numeric(substr(bd.0$Veranlagungsperiode,1,4))
bd.0$Jahr[nchar(as.character(bd.0$Veranlagungsperiode))>4] <- bd.0$Jahr[nchar(as.character(bd.0$Veranlagungsperiode))>4] - 2
bd.2011 <- read.csv("../../data/NP_2011_mitnull.csv",dec=",") %>% filter(Einheit=="Total")
#bd.2010<-bd.0 %>% 
#  filter(Jahr==2010,Einheit=="Total")
#start <- which(names(bd.2010)=="p1")
#end <- which(names(bd.2010)=="p99_99")
...
start <- which(names(bd.2011)=="p1")
end <- which(names(bd.2011)=="p99_99")
percentile <- c(1,5,10,20,25,30,40,50,60,70,75,80,90,95,96,97,98,99,99.5,99.9,99.99)
bd.2011<- data.frame(t(bd.2011[1,start:end]),percentile)
names(bd.2011)[1]=c("inc")
bd.2011 <- rbind(c(0,0),bd.2011)
ival<-bd.2011$inc
perval<-bd.2011$percentile
bd.2011.ind<-unlist(sapply(2:length(ival),function(i){
  len=(perval[i]-perval[i-1])*100
  seq(ival[i-1],ival[i],length.out=len)
}))
bd.2011.ind<-bd.2011.ind/1000
@
...
# Export data to use with Stata
bd2011_2003 <- rbind(data.frame(cbind("bd" = bd.2011.ind,"group"=2011)),data.frame("bd" = bd.2003.ind,"group"=2003))
write.dta(bd2011_2003, file="../../data/stata_data/bd2011_2003.dta")

bd2011_2010 <- rbind(data.frame(cbind("bd" = bd.2011.ind,"group"=2011)),data.frame("bd" = bd.2010.ind,"group"=2010))
write.dta(bd2011_2010, file="../../data/stata_data/bd2011_2010.dta")
@
...
# Export data to use with Stata
ns1993_94 <- rbind(data.frame(cbind("income" = pooled_i,"group"=0)),data.frame("income" = normal_i,"group"=1))
ns1993_94$group <- factor(ns1993_94$group,labels=c("pooled","normal"))
write.dta(ns1993_94, file="../../data/stata_data/ns1993_94.dta")
# Ginis berechnen
gini_allcases <- gini(ns1993_94$income[ns1993_94$group=="pooled"]); gini_allcases
gini_normal <- gini(ns1993_94$income[ns1993_94$group=="normal"]); gini_normal
round(gini_allcases-gini_normal,3)
# 0.013
...
% Variante 2011
<<setup_data_specialcases11>>=
#library(foreign)
#library(ggplot2)
#library(reldist)
normal <- filter(read.csv("../../data/ginis_und_perzentile_normal.csv",sep="\t"),kanton=="CH",steuerperiode==2011)
normal <- select(normal, p1:p99, kanton, steuerperiode, -p96,-p97,-p98)
...
pooled <- read.csv("../../data/NP_2011.csv",dec=",") %>% filter(Einheit=="Total")
...
<<specialcases11, fig.cap='Comparison of normal and special cases 2011',fig.pos='H'>>=
...
# Export data to use with Stata
ns2011 <- rbind(data.frame(cbind("income" = pooled_i,"group"=0)),data.frame("income" = normal_i,"group"=1))
ns2011$group <- factor(ns2011$group,labels=c("pooled","normal"))
write.dta(ns2011, file="../../data/stata_data/ns2011.dta")

#Gini Range
#gini_allcases <- gini(ns2011$income[ns2011$group=="pooled"]); gini_allcases
#gini_normal <- gini(ns2011$income[ns2011$group=="normal"]); gini_normal
#round(gini_allcases-gini_normal,3)
# 0.02
# Es ist quatsch hier die interpolierten Reihen zu nehmen
# Gini 2011 Pooled (BH-Daten): 0.408
# Gini 2011 Normal (unsere ESTV Daten): 0.366
@
...
df <- read.csv("../../data/ginis_und_perzentile_normal.csv",sep="\t")
#library(reshape)
dflong <- melt(df[,c("G_steink","G_steink0","G_steink_halb","kanton","bemessungsjahre")], id=c("kanton","bemessungsjahre"))
names(dflong)[3]<-"Type"
names(dflong)[4]<-"Gini"
levels(dflong$Type)<-c("without non-taxed","including non-taxed","imputed values for non-taxed")
dflong$Type <- factor(dflong$Type, levels=c("including non-taxed","imputed values for non-taxed","without non-taxed"))
#dflong$Gini[dflong$kanton=="CH"&dflong$bemessungsjahre!=1993.5&dflong$bemessungsjahre<2003&dflong$Type=="including non-taxed"]<-NA
#dflong$Gini[dflong$kanton=="CH"&dflong$bemessungsjahre!=1993.5&dflong$bemessungsjahre<2003&dflong$Type=="imputed values for non-taxed"]<-NA
fig_zeros <- ggplot(filter(dflong,kanton=="CH"&
  (((bemessungsjahre<=1993.5|bemessungsjahre>=2003)&Type=="without non-taxed")|
  ((bemessungsjahre==1993.5|bemessungsjahre>=2003)&Type!="without non-taxed"))), 
       aes(x=bemessungsjahre,y=Gini,linetype=Type))+
  scale_y_continuous(limits=c(0.28,0.55))+
  scale_x_continuous(limits=c(1970,2011),breaks=number_ticks(10)) +
  #scale_linetype_manual("Taxable income",values=c("including non-taxed"=1,"imputed values for non-taxed"=2,"without non-taxed"=3))+
  #scale_shape_manual("Taxable income",values=c("including non-taxed"=1,"imputed values for non-taxed"=2,"without non-taxed"=3))+
  geom_line(size=linesize)+
  geom_point(aes(shape=Type), fill = "red",size=pointsize)+
  theme_bw()+
  xlab("Year")+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))+
  ggtitle("Influence of non-taxed")+
  labs(shape="Taxable income")+
  labs(linetype="Taxable income")
fig_zeros

# Gini range
max(filter(df,kanton=="CH") %>% mutate(d_with_without=abs(G_steink-G_steink0)) %>% select(starts_with("d_")),na.rm=TRUE)
max(filter(df,kanton=="CH") %>% mutate(d_with_without=abs(G_steink-G_steink_halb)) %>% select(starts_with("d_")),na.rm=TRUE)
# 0.18
...
# Export data to use with Stata (psteink durch VerfuegbaresEinkommen08 (inc_habe) ersetzt)
habe_bd_2010 <- rbind(data.frame(cbind("income" = habe10$inc_habe,"group"=0,"weight"=habe10$gew)),data.frame("income" = bd.2010.ind,"group"=1,"weight"=1))
habe_bd_2010$group <- factor(habe_bd_2010$group,labels=c("habe","bd"))
write.dta(habe_bd_2010, file="../../data/stata_data/habe_bd_2010.dta")
...
# Dann noch eine extra Version wo die Sozialhilfe abgezogen wird
habe10.married$psteink_ohne_soz<-habe10.married$Bruttoeinkommen08+habe10.married$A31m+habe10.married$A35m- habe10.married$E23

# data-select für "ohne soz"
habe10.married_ohne_soz<-data.frame(habe10.married$VerfuegbaresEinkommen08*12/1000,habe10.married$Bruttoeinkommen08*12/1000,habe10.married$psteink_ohne_soz*12/1000,habe10.married$jahresgewicht)
names(habe10.married_ohne_soz)<-c("inc_habe","brutto","psteink","gew")
habe10.married_ohne_soz$inc_habe[habe10.married_ohne_soz$inc_habe<0]<-0 # One values is -20'000

# data-select für "mit soz"
...
# Export data to use with Stata
#habe_bd_2010_married <- rbind(data.frame(cbind("income" = habe10.married$inc_habe,"group"=0,"weight"=habe10.married$gew)),data.frame("income" = bd.2010.ind,"group"=1,"weight"=1))
#habe_bd_2010_married$group <- factor(habe_bd_2010_married$group,labels=c("habe","bd"))
#write.dta(habe_bd_2010_married, file="../../data/stata_data/habe_bd_2010_married.dta")

# 2011

# Wir brauchen (a) verfügbares Einkommen (b) Bruttoeinkommen und c(Bruttoeinkommen nach Soziversicherungsbeiträgen(A31m) und Monetäre Trasnferausgaben an andere HaushalteA35m)
habe11.married<-habe0911 %>% 
  filter(Familientyp08>130,Jahr08==2011) %>% 
  mutate(psteink=Bruttoeinkommen08+A31m+A35m) 

# und nochmal ohne Sozialhilfe
habe11.married_ohne_soz<-habe0911 %>% 
  filter(Familientyp08>130,Jahr08==2011) %>% 
  mutate(psteink=Bruttoeinkommen08+A31m+A35m-E23) 

habe11.married<-data.frame(habe11.married$VerfuegbaresEinkommen08*12/1000,habe11.married$Bruttoeinkommen08*12/1000,habe11.married$psteink*12/1000,habe11.married$jahresgewicht)
names(habe11.married)<-c("inc_habe","brutto","psteink","gew")
habe11.married$inc_habe[habe11.married$inc_habe<0]<-0 # One values is -500

# und nochmal für ohne_soz
habe11.married_ohne_soz<-data.frame(habe11.married_ohne_soz$VerfuegbaresEinkommen08*12/1000,habe11.married_ohne_soz$Bruttoeinkommen08*12/1000,habe11.married_ohne_soz$psteink*12/1000,habe11.married_ohne_soz$jahresgewicht)
names(habe11.married_ohne_soz)<-c("inc_habe","brutto","psteink","gew")
habe11.married_ohne_soz$inc_habe[habe11.married_ohne_soz$inc_habe<0]<-0 # One values is -500


bd.2011 <- read.csv("../../data/NP_2011_mitnull.csv",dec=",") %>% filter(grepl("Verheiratet",Einheit))
start <- which(names(bd.2011)=="p1")
end <- which(names(bd.2011)=="p99_99")
percentile <- c(1,5,10,20,25,30,40,50,60,70,75,80,90,95,96,97,98,99,99.5,99.9,99.99)
bd.2011<- data.frame(t(bd.2011[1,start:end]),percentile)
...
# Compare ginis for married among tax units and for HBS
round(gini(habe11.married$psteink,w=habe11.married$gew)-gini(bd.2011.ind),3)
# gini_tax-gini_habe diff=-0.066

habe_bd_2011_married <- rbind(data.frame(cbind("income" = habe11.married$inc_habe,"group"=0,"weight"=habe11.married$gew)),data.frame("income" = bd.2011.ind,"group"=1,"weight"=1))
habe_bd_2011_married$group <- factor(habe_bd_2011_married$group,labels=c("habe","bd"))
write.dta(habe_bd_2011_married, file="../../data/stata_data/habe_bd_2011_married.dta")

# und nochmal ohne Sozialhilfe (Vorsicht: hier nehmen wir psteink, vorher verf. einkommen (inc_habe))
habe_bd_2011_married_ohne_soz <- rbind(data.frame(cbind("income" = habe11.married_ohne_soz$psteink,"group"=0,"weight"=habe11.married_ohne_soz$gew)),data.frame("income" = bd.2011.ind,"group"=1,"weight"=1))
habe_bd_2011_married_ohne_soz$group <- factor(habe_bd_2011_married_ohne_soz$group,labels=c("habe","bd"))
write.dta(habe_bd_2011_married_ohne_soz, file="../../data/stata_data/habe_bd_2011_married_ohne_soz.dta")
...
habe10.unmarried<-habe0911 %>% 
  ...
  select(VerfuegbaresEinkommen08,jahresgewicht)
#habe10.unmarried<-data.frame(habe10.unmarried$VerfuegbaresEinkommen08)

names(habe10.unmarried)<-c("inc_habe","gew")
...
# Export data to use with Stata (gewicht hinzugefügt)
habe_bd_2010_unmarried <- rbind(data.frame(cbind("income" = habe10.unmarried$inc_habe,"group"=0,"weight"=habe10.unmarried$gew)),data.frame("income" = bd.2010.ind,"group"=1,"weight"=1))
habe_bd_2010_unmarried$group <- factor(habe_bd_2010_unmarried$group,labels=c("habe","bd"))
write.dta(habe_bd_2010_unmarried, file="../../data/stata_data/habe_bd_2010_unmarried.dta")

#bern kantonal (liegt lokal wegen datenschutz)
try(bern_kt<-read.dta("U:/Persönlich/Forschung/Uni Bern/Steuerdaten_BE_2002_2012.dta"))
try(bern_kt <- read.dta("G:/stick/bern/taxdata_BE_12.dta"))
#> nrow(bern_kt)
#[1] 622368
nrow(filter(bern_kt,von=="2012-01-01",bis=="2012-12-31"))
#[1] 605975 wir filtern ca. 3% unterjährige

try(bern_kt <- filter(bern_kt,VON=="2012-01-01",BIS=="2012-12-31",Satzeink_BUND<=TOTEINK,erm==0))
try(bern_kt <- filter(bern_kt,von=="2012-01-01",bis=="2012-12-31",Satzeink_BUND<=TOTEINK,erm==0))

#names(bern_kt)<-tolower(names(bern_kt))
set0 <- function(x) ifelse(x<0 | is.na(x),0,x)
bern_kt <- bern_kt %>% mutate(primeink2 = set0(hem)+set0(nem)+set0(esm)+set0(sgm)+set0(hef)+set0(nef)+set0(esf)+set0(sgf)+set0(erfolg_s)+set0(erfolg_l)+set0(kk_ertrag)+set0(bg_ertrag)+ set0(wsert)+set0(eink_lieg)+set0(eg_ertrag)+set0(stm)+set0(stf)+set0(lov)+set0(lmv)+set0(lng))

#ALT aber evtl noch mal nüzlich nach der aufbereitung: bern_kt2012 <- bern_kt %>% filter(stj==2012) %>% group_by(hh_id) %>% select(tot_erwerbeink,tot_vermeink,primeink,primeink2,tot_transeink,TOTEINK,Satzeink_BUND,hh_id)

#summarise(bern_kt2012, n=n()) %>% arrange(desc(n))
#habe0911 %>%mutate(anzahl_personen=Anzahl1524Personen08+Anzahl2534Personen08+Anzahl3544Personen08+Anzahl4554Personen08+Anzahl5564Personen08+Anzahl6574Personen08+Anzahl7599Personen08) %>% group_by(anzahl_personen) %>% summarise(n=n())%>%arrange(desc(n))
# gr??ter haushalt 7 personen gross (>=15jahre)
# dort machen wir einen schnitt bei den kantonalen daten um wohnheime etc auszuschlie?en (wie es HABE auch gemacht hat)
#alt: bern_kt2012_plain <- bern_kt2012  %>% filter(n()<=7) #5% werden gefiltert
bern_kt2012_plain <- bern_kt  %>% group_by(hh_id) %>% filter(n()<=7) #5% werden gefiltert #neu
#bern_kt2012 <- bern_kt2012_plain %>% mutate(TOTEINK_hh=sum(TOTEINK)) %>% filter(!duplicated(hh_id))
bern_kt2012 <- bern_kt2012_plain %>% mutate(primeink2_hh=sum(primeink2),eink_satz_dbs_hh=sum(eink_satz_dbs)) %>% filter(!duplicated(hh_id)) #nimmt die erste unique beobachtung und droppt 2. bis x. 

# f?r habe (bern) abz?glich sozialversicherungsbeitr?ge
habe11_bern <- habe0911 %>% filter(Kanton08==2) %>% 
  mutate(income=(Primaereinkommen08)*12/1000,weight=jahresgewicht,group=0) %>% 
  select(income, weight,group)

# export nach stata (ungewichtet)
bern_habe <- rbind(habe11_bern,data.frame("income" = bern_kt2012$primeink2_hh/1000,"group"=1,"weight"=1))
bern_habe$group <- factor(bern_habe$group,labels=c("habe","tax"))
bern_habe <- bern_habe %>% mutate(income=ifelse(income<0,0,income))
write.dta(bern_habe, file="../../data/stata_data/bern_habe.dta")
# Ginis berechnen
gini_habe <- gini(bern_habe$income[bern_habe$group=="habe"],weights=bern_habe$weight[bern_habe$group=="habe"]); gini_habe
gini_tax <- gini(bern_habe$income[bern_habe$group=="tax"]); gini_tax
round(gini_habe-gini_tax,3)
# gini_tax-gini_habe diff=-0.066

# vergleich innerhalb von bern tax data: haushalte vs. plain
# oben wurde plain und nicht plain definiert, das fügen wir jetzt zusammen
# export nach stata (ungewichtet)
bern_kt2012_plain7 <- bern_kt  %>% group_by(hh_id)
bern_kt2012_7 <- bern_kt2012_plain7 %>% mutate(primeink2_hh=sum(primeink2),eink_satz_dbs_hh=sum(eink_satz_dbs)) %>% filter(!duplicated(hh_id)) #nimmt die erste unique beobachtung und droppt 2. bis x. 

bern_plain_hh <- rbind(data.frame("income" = bern_kt2012_plain7$eink_satz_dbs/1000,"group"=0,"weight"=1),data.frame("income" = bern_kt2012_7$eink_satz_dbs_hh/1000,"group"=1,"weight"=1))
bern_plain_hh$group <- factor(bern_plain_hh$group,labels=c("tax units","households"))
bern_plain_hh <- bern_plain_hh %>% mutate(income=ifelse(income<0,0,income))
write.dta(bern_plain_hh, file="../../data/stata_data/bern_plain_hh.dta")
# Ginis berechnen
gini_taxunits <- gini(bern_plain_hh$income[bern_plain_hh$group=="tax units"]); gini_taxunits
gini_hh <- gini(bern_plain_hh$income[bern_plain_hh$group=="households"]); gini_hh
round(gini_taxunits-gini_hh,3)
#diff: 0.058
@

<<bern_kantonale_analysen>>=
# core variables from bern 2002-2012
try(bern <- read.dta("G:/stick/bern/mini_taxdata_BE_all.dta"))
try(bern<-read.dta("P:/WGS/FBS/ISS/Projekte laufend/SNF Ungleichheit/Datengrundlagen/Kantonale Steuerdaten/Bern/taxdata_BE_reduced.dta"))


bern <- filter(bern,Satzeink_KANTON<=TOTEINK,erm==0)
bern$verfeink<-bern$TOTEINK+bern$privattrans_bez-bern$steuertotal

#bern_gini <- bern %>%
#  mutate(stbeink_no_dbs = stbeink_BUND - eink_st_dbs,
#         stbeink_no_tax = stbeink_BUND - steuern_eink) %>% # nur steuern auf einkommen (nicht vermögen) werden abgezogen
#  group_by(stj) %>%
#  summarise(G_TOTEINK = gini(TOTEINK),
#            G_stbeink_BUND = gini(stbeink_BUND),
#            G_stbeink_no_dbs = gini(stbeink_no_dbs),
#            G_stbeink_no_tax = gini(stbeink_no_tax))

bern_gini <- bern %>%
  group_by(stj) %>%
    summarise(G_TOTEINK = gini(TOTEINK),
            G_satzeink_KANTON = gini(Satzeink_KANTON),
            G_TOTEINK = gini(TOTEINK),
            G_verfeink = gini(verfeink))



detach("package:reshape", unload=TRUE) #das ist nötig falls man allen code vorher verwendet hat, da reshape dann geladen ist und reshape und reshape2 gleichnamige funktionen haben die sie intern aufrufen.
library(reshape2)

# wide to long
bern_gini_long <- melt(bern_gini,id.vars="stj",variable.name="Income",value.name="Gini")
names(bern_gini_long)<-c("stj","Income","Gini")
# legend order
levels(bern_gini_long$Income) <- c("total income (earnings, self-employment, capital income)", "taxable income (total income - all deductions)", "disposable income (total income - taxes: federal, cantonal, communal and churches)")
bern_gini_long$Income <- factor(bern_gini_long$Income,levels=c("total income (earnings, self-employment, capital income)", "taxable income (total income - all deductions)", "disposable income (total income - taxes: federal, cantonal, communal and churches)"))
bern_gini_long$Income <- factor(bern_gini_long$Income,levels=c("taxable income (total income - all deductions)","total income (earnings, self-employment, capital income)","disposable income (total income - taxes: federal, cantonal, communal and churches)"))


fig_bern_gini <- ggplot(bern_gini_long, aes(x=stj,y=Gini)) +
  scale_y_continuous(limits=c(0.28,0.6))+
  scale_x_continuous(breaks=number_ticks(5)) +
  geom_point(aes(shape=Income),size=3)+
  geom_line(aes(linetype=Income),size=1)+
  theme_bw()+
  xlab("Year")+
  theme(legend.justification=c(0,1),legend.position=c(0, 0.3))+
  ggtitle("Gini coefficients of different income concepts")
fig_bern_gini

pdf("../figure/fig_bern_gini.pdf",width=10.5,height=6)
fig_bern_gini
dev.off()
@
...
number_ticks <- function(n) {function(limits) pretty(limits, n)}
try(swissGini<-read.csv("../../data/swissGini.csv",header=TRUE,sep=";"))
try(swissGini<-read.csv("C:/Users/hlo1/swiss_inequality_development/data/swissGini.csv",header=TRUE,sep=";"))
...
try(estv <- read.dta("../../data/steuerdaten20141126_stata12.dta"))
try(estv <- read.dta("C:/Users/hlo1/swiss_inequality_development/data/steuerdaten20141126_stata12.dta"))
...
estv_gini <- estv %>% 
  filter(kanton=="CH",
         eink_art=="Nach steuerbarem Einkommen",
         fall=="Normalfälle",
         anz_pflichtige>0,
         bemessungsjahre<=1993.5|bemessungsjahre>=2003) %>%
        # steuerperiode!=1945.5) %>% 
...
all_gini_long$Data <- factor(all_gini_long$Data,levels=c("FTA","LIS","EU.SILC","HBS"))

benplot <- ggplot(data=na.omit(all_gini_long),
  aes(y=Gini,x=Year,shape=Data,linetype=Data))+
  ylab("Gini") +
  scale_y_continuous(limits=c(0.18,0.45)) +
  scale_x_continuous(limits=c(1945,2012),breaks=number_ticks(10)) +
  geom_point(size=4)+
  geom_line(size=1)+
...
benplot74 <- ggplot(data=na.omit(all_gini_long),
  aes(y=Gini,x=Year,shape=Data,linetype=Data))+
  ylab("Gini from different sources") +
  scale_y_continuous(limits=c(0.18,0.45)) +
  scale_x_continuous(limits=c(1973,2012),breaks=number_ticks(10)) +
  geom_point(size=pointsize)+
  geom_line(size=linesize)+
  theme_bw()+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))

png("../figure/ben_plot.png",width=800,height=450)
benplot
dev.off()

pdf("../figure/ben_plot.pdf",width=10.5,height=6)
benplot
dev.off()

pdf("../figure/ben_plot74.pdf",width=10.5,height=6)
benplot74
dev.off()

# kleiner einschub: verschiedene masse
library(IC2)
getAtkinson <- function(x,w){
  calcAtkinson(x,w=w)$ineq$index
}
getTheil <- function(x,w){
  calcGEI(x,w=w,alpha=1)$ineq$index
}
...
estv_masse <- estv %>% 
  ...
  group_by(factor(bemessungsjahre)) %>%
  mutate(mean_steink=steink/anz_pflichtige) %>%
  summarise(Gini=gini(mean_steink,weights=anz_pflichtige),Atkinson=getAtkinson(mean_steink,w=anz_pflichtige),Theil=getTheil(mean_steink,w=anz_pflichtige))
estv_masse$id<-1
names(estv_masse)<-c("Year","Gini","Atkinson","Theil","id")
estv_masse<-estv_masse[as.numeric(estv_masse$Year)>4,]
try(detach("package:reshape", unload=TRUE))
library(reshape2)
estv_masse_long <- melt(estv_masse,id=c("id","Year"))

estv_masse_long$Measure <- factor(estv_masse_long$variable)
estv_masse_long$Year <- as.double(as.character(estv_masse_long$Year))
estv_masse_long <- estv_masse_long %>% group_by(Measure) %>% mutate(log_value=log(value)-log(value)[1])

fig_measures <- ggplot(estv_masse_long, aes(x=Year,y=log_value,shape=Measure))+
  scale_y_continuous(limits=c(-0.7,0.45))+
  scale_x_continuous(limits=c(1945,2011),breaks=number_ticks(10)) +
  geom_point(aes(shape=Measure),size=3)+
  geom_line(aes(linetype=Measure))+
  theme_bw()+
  xlab("Year")+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))+
  ggtitle("Using different inequality measures")
fig_measures

# output als 4er grafik
library(gridExtra)
png("../figure/figure1234.png",width=1200,height=675)
grid.arrange(fig_incomes,fig_equiv,fig_measures,fig_zeros)
dev.off()

#output als einzelne pdfs
pdf("../figure/fig_incomes.pdf",width=10.5,height=6)
fig_incomes
dev.off()
pdf("../figure/fig_equiv.pdf",width=10.5,height=6)
fig_equiv
dev.off()
pdf("../figure/fig_zeros.pdf",width=10.5,height=6)
fig_zeros
dev.off()
pdf("../figure/fig_measures.pdf",width=10.5,height=6)
fig_measures
dev.off()
@
