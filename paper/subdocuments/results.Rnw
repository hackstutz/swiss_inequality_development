...
g10 <- reldist(y=bd.2010.ind$inc, yo=bd.2003.ind$inc,
               ...
               xlab="Proportion of 2002 Distribution")
...
For more recent periods however we can compare already calculated percentiles provided by the FTA which are based on both special and normal cases to normal cases alone.
...
axis(side = 1)
axis(side = 2)
fig1legend <- list(x=c(100,100),y=c(0.0116031,0.0116031))
legend(fig1legend,lty=1:2,cex=0.5, bty="n",
...
par(mfrow=c(1,3))
reldist(y=pooled_i, yo=normal_i,
               smooth=0.4, ci=FALSE,
               yolabs=seq(-1,3,by=0.5),
               ylim=c(0.5,5.0),
               bar=TRUE, quiet=FALSE,
...
<<specialcases10, echo=FALSE,warning=FALSE>>=
library(foreign)
library(ggplot2)
#library(reldist)
...
axis(side = 1)
axis(side = 2)
fig1legend <- list(x=c(120,120),y=c(0.0116031,0.0116031))
legend(fig1legend,lty=1:2,cex=0.5, bty="n",
       legend=c("normal cases","pooled, normal and special cases"))
density2 <- density(pooled_i)
lines(x = (density2$x), y = density2$y, type = "l",lty=2)
