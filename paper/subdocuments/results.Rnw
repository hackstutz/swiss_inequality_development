...
bern_kt2012 <- bern_kt %>% filter(stj==2012) %>% group_by(hh_id) %>% select(tot_erwerbeink,tot_vermeink,primeink,tot_transeink,TOTEINK,Satzeink_BUND,hh_id)
...
bern_kt2012 <- bern_kt2012_plain %>% mutate(primeink_hh=sum(primeink),Satzeink_BUND_hh=sum(Satzeink_BUND)) %>% filter(!duplicated(hh_id)) #nimmt die erste unique beobachtung und droppt 2. bis x. 
...
bern_habe <- rbind(habe11_bern,data.frame("income" = bern_kt2012$primeink_hh/1000,"group"=1,"weight"=1))
