...
<<different_ginis, fig.cap='',fig.pos='H'>>=
...
<<equivalencescale, fig.cap='',fig.pos='H'>>=
number_ticks <- function(n) {function(limits) pretty(limits, n)}
...
<<reldist20032010, echo=FALSE, fig.cap='',fig.pos='H'>>=
...
<<specialcases9394, fig.cap='',fig.pos='H'>>=
...
<<specialcases10, fig.cap='',fig.pos='H'>>=
...
<<with_without_zeros_Gini,warning=FALSE, fig.cap='',fig.pos='H'>>=
df <- read.csv("../../data/ginis_und_perzentile_normal.csv",sep="\t")
#library(reshape)
...
<<with_without_zeros_RelDist_mitESTV,warning=FALSE, fig.cap='',fig.pos='H'>>=
...
<<polindexzeros2, results='asis'>>=
polTable(bd.2010.ind,bd.0.2010.ind,tablabel="polzeros2")
@
...
<<BruelhartvsHABE,warning=FALSE, fig.cap='',fig.pos='H'>>=
#library(ggplot2)
...
#1918 fixen
estv <- within(estv, {
    steink[steuerperiode==1918]<-(eink_ll[steuerperiode==1918]+eink_ul[steuerperiode==1918])/2*anz_pflichtige[steuerperiode==1918]
})
...
all_gini_long$annotation[all_gini_long$Year==1939.5] <- "Tolle\nSteuererhöhung"
all_gini_long$annotation[all_gini_long$Year==1955.5] <- "Tolle\nSteuersenkung"

annotations <- data.frame("Year"=c(1939.5,1955.5),"label"=c("Tolle\nSteuererhöhung","Tolle\nSteuersenkung"))
...
ggplot(data=na.omit(all_gini_long),
  ...
  geom_point(size=3)+
  geom_line()+
  geom_text(aes(label=annotation,y=0.29),color="#555555",size=3)+
  geom_segment(aes(x = 1939.5, y = 0.295, xend = 1939.5, yend = Gini[Year==1939.5]),color="#aaaaaa") +
  geom_segment(aes(x = 1955.5, y = 0.295, xend = 1955.5, yend = Gini[Year==1955.5]),color="#aaaaaa") +
  theme_bw()+
  theme(legend.justification=c(0,1),legend.position=c(0, 1))
