...
ggplot(df[df$steuerperiode>=1995,], aes(x=steuerperiode,y=zeros))+geom_line()+facet_wrap(~kanton)+geom_line(aes(y=G),color="blue")
@
...
<<correct_gini>>=
fit <- lm(G~kanton+kanton:zeros+factor(steuerperiode)+factor(steuerperiode):zeros,data=df)
summary(fit)
@
...
<<magnitude_of_bias>>=
reduced_fit <- lm(G~kanton+factor(steuerperiode),data=df)
summary(reduced_fit)
@
...
<<echo=FALSE>>=
# checking the impact of zeros on an artificial distribution
# generate a few differently scewed distributions and introduce zeros.
x02 <- rgamma(1000,shape=0.2,scale=5)
x05 <- rgamma(1000,shape=0.5,scale=2)
x1 <- rgamma(1000,shape=1,scale=1/1)
x2 <- rgamma(1000,shape=2,scale=1/2)
x3 <- rgamma(1000,shape=3,scale=1/3)
x4 <- rgamma(1000,shape=4,scale=1/4)
x1 <- x1[order(x1)]
x2 <- x2[order(x2)]
x3 <- x3[order(x3)]
x4 <- x4[order(x4)]
# delete the lowest 20%
x02_20 <- x02
x02_20[1:200] <- NA
x05_20 <- x05
x05_20[1:200] <- NA
...
gini(x02)-gini(na.omit(x02_20))

gini(x05)-gini(na.omit(x05_20))

gini(x1)-gini(na.omit(x1_20))

gini(x2)-gini(na.omit(x2_20))

gini(x3)-gini(na.omit(x3_20))

gini(x4)-gini(na.omit(x4_20))
# gini inceases
@
...
ggplot(df[df$kanton=="CH",], aes(x=steuerperiode,y=G_steink))+geom_line()+facet_wrap(~kanton)+geom_line(aes(y=G_reink),color="blue")+geom_line(aes(y=G_taxed),color="green")
ggplot(df, aes(x=steuerperiode,y=G_steink))+geom_line()+facet_wrap(~kanton)+geom_line(aes(y=G_reink),color="blue")+geom_line(aes(y=G_taxed),color="green")
@
