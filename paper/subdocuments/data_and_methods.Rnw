...
% Die Deductions sind noch nicht gänzlich sauber abgebildet. Auf der Bilanz-Seite steht, vom steuerbaren Einkommen kommt man, indem vom Reineinkommen die Sozialabzüge(Kinder- und Fremdbetreung) sowie der Abzug für unterstützungsbedürftige Personen abzogen werden. 
...
% letzter Abschnitt muss noch auf die Brülhart-Daten hinweisen. Damit können wir eine äquivalenzeinkommen betrachten.
% Auch müsste in diesem Abschnitt auf die Probleme der Haushaltsunterscheidung eingegangen werden (oder ist im Abschnitt Population coverage? Dann müsste obiger Punkt auch da erwähnt werden.)
